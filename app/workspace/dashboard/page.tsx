  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const userUrlsData = response.data?.data?.userUrls as userUrl[];
        setUserUrls(userUrlsData || []);
      } catch (error) {
  ...
  return (
    <main className="w-full min-h-[93vh]">
      <div className="w-full pt-3 lg:px-18 px-2 flex flex-col h-full">
        ...
        {/* Quick Shorten Form */}
        <form onSubmit={handleShortenSubmit} className="w-full mt-7 flex gap-3">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="w-full border rounded-sm mt-3 border-border/70 overflow-hidden">
