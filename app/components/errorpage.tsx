...
export default  function Errorpage({type, code}:errorpageTypes){
    ...
    useEffect(()=>{
        const timer = setInterval(()=>{
                setCount((e)=>{
                    if(e>0){return e-1}else{return 0}
                })
        },1000)
        if(count == 0){
    ...
    return (
        ...
        <main className="w-full h-screen flex flex-col ">
            <header className="w-full h-16 border flex items-center justify-center font-extrabold bg-card/80 font-sans text-3xl ">{NAME}</header>
            <div className="flex flex-col gap-3 items-center  grow border">
              ...
              {type==="maxclicks"&&(
               ...
               <div className="text-md text-center max-w-sm text-muted-foreground mt-7">
                    Know who where and when your link was visited for free with <Link href="/" className="text-primary font-medium font-sans hover:text-primary/70 transition-colors">{NAME}</Link>
                </div>
                
                <div className="w-full flex flex-col gap-3 items-center mt-5">
                    <div className="text-sm  font-bold text-muted-foreground font-sans flex gap-1 items-center justify-center">redirecting home in <span className="text-md font-extrabold font-serif text-primary"> {count}</span></div>
                    <Link href="/" >
